...
public class HomeSentNotificationPendingViewModel extends AndroidViewModel {

    private MutableLiveData<List<Contact>> mContactRequestList;

    public String sentRequestNumber;

    public HomeSentNotificationPendingViewModel(@NonNull Application application) {
        super(application);
    ...
    private void handleResult(final JSONObject response) {
        ...
        try {
            if (!response.has("rowCount")) {
                ...
                sentRequestNumber = String.valueOf(response.getInt("rowCount"));
            }
